...
check(styles.includes(".editorSidebarEngines{margin-top:auto")&&styles.includes(".editorSidebarEngineList{width:100%;display:flex;flex-direction:column")&&styles.includes(".editorSidebarEngineName{")&&styles.includes("font-size:clamp(4.5px")&&styles.includes(".editorLayers.engineDensityUltra")&&styles.includes("font-size:3.65px")&&workspace.includes('data-engine-count={selectedEngineLabels.length}')&&workspace.includes('"--engine-count":Math.max(1,selectedEngineLabels.length)')&&workspace.includes('selectedEngineLabels.length>=24?"engineDensityUltra"'),"sidebar engine block and full sidebar must adapt density for large selected-engine counts");
