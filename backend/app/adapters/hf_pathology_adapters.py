...
class HFUltralyticsAdapter(EngineAdapter):
    def __init__(self,eid,name,repo_id,filename,task,description,license_name,source_url):
        self.repo_id=repo_id; self.filename=filename; self.model=None
        self.meta=AdapterMeta(
            eid,name,"Ultralytics / Hugging Face",task,description,
            False,True,"public_shm_checkpoint",source_url,license_name
        )
    ...
    def predict(self,image):
        conf=float(os.getenv("SHM_PUBLIC_YOLO_CONF",".25"))
        r=self._load().predict(np.asarray(image.convert("RGB")),verbose=False,conf=conf)[0]
        ...
        if masks is not None and masks.size:
            mask_scale=(image.width*image.height)/(masks.shape[-2]*masks.shape[-1])
        for i,(box,score,cls) in enumerate(zip(boxes,confs,classes)):
            label=str(names.get(int(cls),cls))
            area=None
            if masks is not None and i<len(masks):
                area=float(masks[i].sum()*mask_scale)
                mask_ratios[label]=mask_ratios.get(label,0.0)+area/(image.width*image.height)
        ...
        return EngineResult(engine_id=self.meta.id,name=self.meta.name,task=self.meta.task,status="ok",
            detections=ds,overlay_png_base64=png_b64(overlay),metrics=metrics)
    ...
    def __init__(self,in_channels=3,out_channels=1,depth=4,start_filters=64):
        ...
        def block(a,b,drop=.2):
            return nn.Sequential(
                nn.Conv2d(a,b,3,padding=1),nn.BatchNorm2d(b),nn.ReLU(inplace=True),nn.Dropout2d(drop),
        ...
        for i in range(depth-1,-1,-1):
            out=start_filters*(2**i)
            self.upconvs.append(nn.ConvTranspose2d(current,out,2,2))
            self.decoders.append(block(out*2,out))
            current=out
...
class SamirUNetCrackAdapter(EngineAdapter):
    meta=AdapterMeta(
        "unet_public_crack","U-Net Concrete Crack (public)","PyTorch / Hugging Face","semantic_segmentation",
        ...
        "https://huggingface.co/samir-mohamed/concrete-crack-segmentation","MIT"
    )
    ...
    def _load(self):
        if self.model is None:
            path=hf_hub_download(
                ...
                filename="unet_model_weights.pth",cache_dir=CACHE_DIR
            )
            m=ImprovedUNet(3,1,4,64)
            state=torch.load(path,map_location="cpu",weights_only=True)
            m.load_state_dict(state,strict=True); self.model=m.eval()
        return self.model
    def predict(self,image):
        src=image.convert("RGB"); resized=src.resize((256,256),Image.Resampling.LANCZOS)
        ...
        prob_np=np.asarray(prob,dtype=np.float32)/255.0; mask=prob_np>=threshold
        rgb=np.asarray(src,dtype=np.float32); out=rgb.copy()
        out[mask]=.55*rgb[mask]+.45*np.array([255,70,30],dtype=np.float32)
        ...
        return EngineResult(engine_id=self.meta.id,name=self.meta.name,task=self.meta.task,status="ok",
            detections=ds,overlay_png_base64=png_b64(Image.fromarray(np.clip(out,0,255).astype(np.uint8))),
            metrics={"crack_area_ratio":round(float(ratio),6),"mean_probability":round(float(prob_np.mean()),6),
                     "max_probability":round(float(prob_np.max()),6),"threshold":threshold,
                     "source_repo":"samir-mohamed/concrete-crack-segmentation"})

def public_pathology_catalog():
    return [
        HFUltralyticsAdapter(
            "yolov8_public_crack","YOLO Crack Detector (public)",
            "Utbird/crack_detection","crack.pt","detection",
            "YOLO treinado especificamente para fissuras em superfícies de infraestrutura.",
            "Apache-2.0","https://huggingface.co/Utbird/crack_detection"
        ),
        HFUltralyticsAdapter(
            "yolov8_public_damage_seg","YOLOv8 Structural Damage Segmentation (public)",
            ...
            "Apache-2.0","https://huggingface.co/iloveass/kompon-damage-severity-detection"
        ),
