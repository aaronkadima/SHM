...
export default function AnalysisWorkspace({appInfo=null,executionIssue="",referenceValidating=false,selectedEngineLabels=[],file,prev,referenceFile,referencePrev,spatialRgbFile=null,spatialRgbPrev=null,onSpatialRgbFile=null,spatialRegistration=null,spatialRgbAnalysis=null,spatialPathologyRecords=[],onSpatialPathologyRecords=null,onSolveSpatialRegistration=null,referenceInspectionId,referenceInspectionMeta,inspectionMeta,res,busy,progress,selected,onFile,onReferenceFile,onRun,onCancel,onSettings,error,onExport,onExportCsv,onExportMap,onExportCdm,onExportCdm3=null}){
  ...
  return <section className="analysisEditor" aria-label="Workspace de análise">
    <div className="editorTop">
      ...
      <div className="editorTopActions">
        ...
        {kind==="3d"&&isCdm3SpatialAsset(file)&&selected.length===1&&selected[0]==="cdm_3"&&<button className={spatialRgbFile?"spatialRgbAttached":""} disabled={busy} title="Anexar imagem RGB para registro fotogramétrico 2D→3D" onClick={()=>spatialRgbPicker.current?.click()}><ImagePlus size={16}/> {spatialRgbFile?"RGB: "+spatialRgbFile.name:"Imagem RGB"}</button>}
    ...
    <div className="editorBody">
      ...
      {layersOpen&&<><button type="button" className="editorLayersScrim" aria-label="Fechar painel de camadas" title="Fechar camadas" onClick={()=>setLayersPanelOpen(false)}/><aside className={"editorLayers "+(selectedEngineLabels.length>=24?"engineDensityUltra":selectedEngineLabels.length>=12?"engineDensityDense":"")} data-engine-count={selectedEngineLabels.length} data-layer-width={layersWidth} style={{width:layersWidth,"--engine-count":Math.max(1,selectedEngineLabels.length)}}><div className="editorLayersContent"><div className="editorPanelTitle"><b>Camadas</b><button aria-label="Recolher painel de camadas" title="Recolher camadas" onClick={()=>setLayersPanelOpen(false)}><ChevronLeft size={17}/></button></div>
        ...
        {kind==="3d"&&spatialRgbFile&&<div className="layerRow referenceLayer spatialRgbLayer"><span>◉</span><span>RGB externo <small>{spatialRgbFile.name}</small><em className={"referenceCompatibility "+(spatialRegistration?.registration?"pass":"warning")}>{spatialRegistration?.registration?"registro 2D→3D resolvido":"registro 2D→3D pendente"}</em></span><button className="layerClear" aria-label="Remover imagem RGB espacial" disabled={busy} title="Remover imagem RGB" onClick={()=>onSpatialRgbFile?.(null)}><X size={13}/></button></div>}
