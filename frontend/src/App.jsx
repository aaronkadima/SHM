...
import{Upload,Play,CheckCircle2,AlertTriangle,Clock3,Layers3,Server,Save,Wifi,WifiOff,FlaskConical}from"lucide-react";
...
export default function App(){
 ...
 async function loadEngines(base=api){
   ...
   try{
     ...
     const x=await r.json();setEng(x);
     setSel(new Set(x.filter(e=>e.ready&&e.recommended).map(e=>e.id)));
     setOnline(true);
 ...
 const recommended=useMemo(()=>eng.filter(e=>e.ready&&e.recommended).length,[eng]);
 ...
 function selectReady(){setSel(new Set(eng.filter(e=>e.ready).map(e=>e.id)))}
 function selectRecommended(){setSel(new Set(eng.filter(e=>e.ready&&e.recommended).map(e=>e.id)))}
 async function run(){
 ...
 return <main>
   <header><div><div className="eye"><Layers3 size={16}/> SHM · OAEs · COMPUTER VISION</div><h1>SHM Vision Lab</h1><p>Uma imagem. Vários motores. Resultados normalizados lado a lado.</p></div><div className="sum"><b>{eng.length}</b><span>motores</span><b>{ready}</b><span>prontos</span><b>{recommended}</b><span>recomendados agora</span></div></header>
   ...
   <div className="scienceNote"><FlaskConical size={17}/><span><b>Comparação cientificamente rastreável:</b> motores zero-shot podem localizar patologias por texto sem treino específico. YOLO/RT-DETR genéricos continuam disponíveis, mas só devem ser interpretados como detectores SHM quando houver checkpoint treinado para as mesmas classes.</span></div>

   <section className="work"><div className="left"><label className="drop"><input type="file" accept="image/*" onChange={e=>pick(e.target.files?.[0])}/>{prev?<img src={prev}/>:<><Upload size={34}/><b>Selecionar imagem da inspeção</b><span>JPG, PNG ou WEBP</span></>}</label><button className="run" disabled={!file||busy||!sel.size||!online} onClick={run}><Play size={18}/>{busy?"Processando motores em fila segura...":"Executar comparação"}</button>{err&&<div className="error">{err}</div>}</div><div className="panel"><div className="pt"><h2>Motores</h2><div className="selectBtns"><button onClick={selectRecommended}>Recomendados</button><button onClick={selectReady}>Prontos</button></div></div><div className="elist">{eng.length?eng.map(e=><label className={"engine "+(!e.ready?"disabled":"")} key={e.id}><input type="checkbox" disabled={!e.ready} checked={sel.has(e.id)} onChange={()=>toggle(e.id)}/><div><b>{e.name}{e.recommended&&<em> recomendado</em>}</b><span>{e.family} · {e.task.replaceAll("_"," ")} · {modeLabel(e)}</span><small>{e.description}</small>{!e.ready&&e.reason&&<small className="reason">{e.reason}</small>}</div>{e.ready?<CheckCircle2 className="ok" size={18}/>:<AlertTriangle className="warn" size={18}/>}</label>):<div className="engineEmpty">Conecte o backend online para carregar os motores.</div>}</div></div></section>
   {res&&<section className="results"><h2>Comparação</h2><p>{res.image_width} × {res.image_height}px · mesma entrada para todos os motores selecionados</p><div className="grid">{res.results.map(r=><Card key={r.engine_id} r={r}/>)}</div></section>}
 </main>
}
