    ...
    <article className="surface historySurface">
      ...
      {history?.length?<div className="historyList">
        ...
        {history.map(h=><div className="historyRow" key={h.id}>
          ...
          <span>{h.inspection?.source_id||h.file_meta?.name||"—"}{h.summary?.has_reference_image?<small>t0: {h.reference_file_meta?.name||"referência salva"}{h.reference_inspection_id?" · vinculada "+String(h.reference_inspection_id).slice(0,10):" · manual/externa"}</small>:null}</span>
