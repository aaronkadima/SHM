...
try{
  ...
  await evaluate(request,'(()=>{const input=document.querySelector(\'.editorTopActions input[type="file"]\');if(!input)throw new Error("analysis file input not found at "+location.href+" body="+document.body?.innerText?.slice(0,300));const xyz=["100 200 10 255 20 10","101 200 10.5 230 80 25","102 201 11 30 255 60","103 202 12 20 80 255","104 203 12.5 240 220 40"].join("\\n");const file=new File([xyz],"cdm3-spatial-smoke.xyz",{type:"text/plain"});const dt=new DataTransfer();dt.items.add(file);input.files=dt.files;input.dispatchEvent(new Event("change",{bubbles:true}));return true})()');
  ...
  const state=await waitUntil(request,'(()=>{const row=document.querySelector(".editorResultRow");const title=[...document.querySelectorAll(".editorCdmTitle")].map(x=>x.textContent||"").find(x=>x.includes("CDM-3 · ativo espacial"))||"";const progress=document.querySelector(".editorProgress");const model=document.querySelector(".modelViewport");const visual=document.querySelector(".pointCloudModes select");const visualNote=document.querySelector(".pointCloudModes span")?.textContent?.trim()||"";const status=document.querySelector(".editorStatusMessage")?.textContent?.trim()||"";if(row&&title&&!progress&&model&&visual)return {row:row.textContent?.trim()||"",title,status,badge:document.querySelector(".editorTypeBadge")?.textContent?.trim()||"",engine:document.querySelector(".editorEngineState")?.textContent?.trim()||"",visual:visual.value,visualNote};return null})()',"CDM-3 spatial result",60000);
  ...
  if(state.visual!=="rgb")throw new Error("RGB was not selected for colored point cloud: "+JSON.stringify(state));
  if(!/RGB por ponto/i.test(state.visualNote))throw new Error("RGB point-cloud guidance missing: "+JSON.stringify(state));
