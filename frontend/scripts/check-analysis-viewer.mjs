...
check(styles.includes("@media(max-width:900px){")&&styles.includes(".editorLayers{position:absolute;left:56px;top:0;bottom:0;z-index:22")&&styles.includes(".editorLayerResizeHandle{display:none}")&&styles.includes("@media(max-width:560px){.editorTop{height:52px;padding:0 8px;gap:8px}")&&styles.includes(".editorBrand strong{display:none}")&&styles.includes(".editorTopActions button{width:32px;min-width:32px;height:32px;min-height:32px")&&styles.includes(".editorLayers{width:min(238px,calc(100% - 56px))!important}")&&styles.includes(".editorFloating{right:8px;width:calc(100% - 16px);min-width:0;max-width:calc(100% - 16px)}")&&styles.includes(".editorLayers~.editorViewport .editorFloating{visibility:hidden;opacity:0;pointer-events:none}")&&styles.includes(".editorLayers~.editorViewport .editorResultsTab{visibility:hidden;pointer-events:none}"),"mobile analysis must compact the top bar, size Results from the canvas, prevent viewport overflow and show only one overlay panel at a time");
