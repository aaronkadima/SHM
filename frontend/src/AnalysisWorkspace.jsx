    ...
    <div className="editorBody">
      ...
      <div className={"editorViewport "+(fileDragActive?"fileDragActive":"")} ref={surface} onDragEnter={fileDragEnter} onDragOver={fileDragOver} onDragLeave={fileDragLeave} onDrop={fileDrop}>
        ...
        {resultOpen&&(busy||results.length>0)&&<div ref={resultPanel} className="editorFloating" data-position-x={position.x} data-position-y={position.y} data-panel-width={resultPanelSize.width} data-panel-height={resultPanelSize.height||""} style={{transform:`translate(${position.x}px,${position.y}px)`,"--result-panel-width":resultPanelSize.width+"px",...(resultPanelSize.height?{"--result-panel-height":resultPanelSize.height+"px"}:{})}}>
          ...
          {results.map(r=>{const area=Number(r.metrics?.crack_area_ratio??r.metrics?.model_crack_area_ratio);const hasArea=Number.isFinite(area)&&area>=0;return <button key={r.engine_id} className={"editorResultRow "+(chosen?.engine_id===r.engine_id?"active":"")} onClick={()=>{setActive(r.engine_id);setVisible(v=>({...v,[r.engine_id]:true}))}}><span className="editorResultEngine">{r.name}</span><span className="editorResultMetric"><small>{hasArea?"Área fissura":"Achados"}</small><b>{hasArea?(area*100).toFixed(area<.001?3:2)+"%":(r.detections?.length||0)}</b></span><span className="editorResultMetric"><small>Latência</small><b>{Number(r.latency_ms||0).toFixed(0)} ms</b></span></button>})}
