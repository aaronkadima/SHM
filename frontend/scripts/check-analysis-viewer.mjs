...
check(settingsStyles.includes(".settingsEngines{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-top:19px;align-items:start;overflow:visible}")&&!settingsStyles.includes("max-height:520px;overflow:auto"),"engine catalog must grow inside settings content instead of creating a nested scrollbar");
