...
check(styles.includes(".editorSidebarEngines{margin-top:auto")&&styles.includes(".editorSidebarEngineList{width:100%;display:flex;flex-direction:column")&&styles.includes(".editorSidebarEngineName{")&&styles.includes("font-size:clamp(4.5px")&&workspace.includes('data-engine-count={selectedEngineLabels.length}')&&workspace.includes('"--engine-count":Math.max(1,selectedEngineLabels.length)'),"sidebar engine block must stay vertically aligned and compact with adaptive density for multiple selected engines");
