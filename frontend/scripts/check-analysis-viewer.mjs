...
check(styles.includes("@media(max-width:900px){")&&styles.includes(".editorLayers{position:absolute;left:56px;top:0;bottom:0;z-index:22")&&styles.includes(".editorLayerResizeHandle{display:none}")&&styles.includes("@media(max-width:560px){.editorTop{height:52px;padding:0 8px;gap:8px}")&&styles.includes(".editorBrand strong{display:none}")&&styles.includes(".editorTopActions button{width:32px;min-width:32px;height:32px;min-height:32px")&&styles.includes(".editorLayers{width:min(238px,calc(100% - 56px))!important}"),"mobile analysis must compact the top bar and use a fixed overlay sidebar without viewport overflow");
