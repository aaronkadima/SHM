...
check(styles.includes(".editorFloating{position:absolute;right:18px;top:82px;width:360px;max-width:calc(100% - 36px);max-height:75%;box-sizing:border-box")&&styles.includes("min-width:340px")&&workspace.includes("function clampResultPosition(next)")&&workspace.includes("panel.offsetLeft")&&workspace.includes("viewport.clientWidth")&&workspace.includes("setPosition(clampResultPosition")&&workspace.includes("function moveResultPanelKey(e)")&&workspace.includes('aria-label="Mover painel de resultados"')&&workspace.includes('title="Arraste ou use setas, Home e End para mover"')&&workspace.includes('e.key==="Home"||e.key==="End"')&&workspace.includes("ref={resultPanel}")&&workspace.includes("data-position-x={position.x}"),"floating results container must remain pointer/keyboard movable and resizable while clamped to the analysis viewport");
