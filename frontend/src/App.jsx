...
export default function App(){
  ...
  return <div className={"appShell "+(["analysis","settings"].includes(activeView)?"editorShell":"")}>
    ...
    {activeView==="analysis"&&<>
    <AnalysisWorkspace file={file} prev={prev} res={res} busy={busy} progress={progress} selected={selected} selectedEngines={engines.filter(e=>sel.has(e.id))} onFile={pick} onRun={run} onCancel={jobId?cancelRun:null} onSettings={()=>navigate("settings")} error={err} onExport={()=>res&&exportJson(res)} onExportCsv={()=>res&&exportCsv(res)} onExportMap={()=>res&&downloadConsensus(res)}/>
    </>}
