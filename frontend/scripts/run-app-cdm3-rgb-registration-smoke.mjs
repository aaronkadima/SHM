...
try{
  ...
  await waitUntil(request,'(()=>{const badge=document.querySelector(".editorTypeBadge")?.textContent||"";const actions=document.querySelector(".editorTopActions")?.textContent||"";return badge.includes("CDM-3 espacial")&&actions.includes("Adicionar fotos")})()',"external RGB action");
  await evaluate(request,'(async()=>{const input=document.querySelector(\'.editorTopActions input[type="file"][multiple]\');if(!input)throw new Error("multiview RGB input missing");const make=async(name,a,b)=>{const canvas=document.createElement("canvas");canvas.width=8;canvas.height=8;const ctx=canvas.getContext("2d");ctx.fillStyle=a;ctx.fillRect(0,0,8,8);ctx.fillStyle=b;ctx.fillRect(1,1,6,2);ctx.fillStyle="#3e5f6e";ctx.fillRect(2,4,4,3);const blob=await new Promise(resolve=>canvas.toBlob(resolve,"image/png"));if(!blob)throw new Error("could not create RGB fixture");return new File([blob],name,{type:"image/png"})};const dt=new DataTransfer();dt.items.add(await make("bridge-frame-a.png","#785a42","#d8d0bd"));dt.items.add(await make("bridge-frame-b.png","#6b594c","#c8c3b5"));input.files=dt.files;input.dispatchEvent(new Event("change",{bubbles:true}));return true})()');
  await waitUntil(request,'(()=>{const preview=document.querySelector(".spatialRgbPreview");const layers=[...document.querySelectorAll(".spatialRgbLayer")];const actions=document.querySelector(".editorTopActions")?.textContent||"";return !!preview&&layers.length===2&&actions.includes("Fotos 3D · 2")})()',"linked external RGB source");
  ...
  const state=await waitUntil(request,'(()=>{const summary=document.querySelector(".editorCdmSummary")?.textContent||"";const row=document.querySelector(".editorResultRow")?.textContent||"";const progress=document.querySelector(".editorProgress");if(!progress&&summary.includes("bridge-frame-a.png")&&summary.includes("PnP/RANSAC pendente"))return {summary,row,preview:!!document.querySelector(".spatialRgbPreview"),layers:document.querySelectorAll(".spatialRgbLayer").length};return null})()',"CDM-3 external RGB registration result",60000);
  if(!state.preview||state.layers!==2)throw new Error("Multiview RGB sources disappeared: "+JSON.stringify(state));
  ...
  console.log("APP_CDM3_RGB_REGISTRATION_SMOKE_PASS",JSON.stringify({...state,multiview:true}));
