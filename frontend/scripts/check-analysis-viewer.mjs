...
check(styles.includes("@media(max-width:900px){")&&styles.includes(".editorLayers{position:absolute;left:56px;top:0;bottom:0;z-index:22")&&styles.includes(".editorLayerResizeHandle{display:none}")&&styles.includes("@media(max-width:560px){.editorLayers{width:min(238px,calc(100% - 56px))!important}"),"mobile analysis must use a fixed overlay sidebar that preserves canvas width and disables the resize handle");
