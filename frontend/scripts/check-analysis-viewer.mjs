...
check(styles.includes("@media(max-width:900px){")&&styles.includes(".editorLayers{position:absolute;left:56px;top:0;bottom:0;z-index:22")&&styles.includes(".editorLayerResizeHandle{display:none}")&&styles.includes("@media(max-width:560px){.editorTop{height:52px;padding:0 8px;gap:8px}")&&styles.includes(".editorBrand strong{display:none}")&&styles.includes(".editorTopActions button{width:32px;min-width:32px;height:32px;min-height:32px")&&styles.includes(".editorLayers{width:min(238px,calc(100% - 56px))!important}")&&styles.includes(".editorLayers~.editorViewport .editorFloating{visibility:hidden;opacity:0;pointer-events:none}")&&styles.includes(".editorLayers~.editorViewport .editorResultsTab{visibility:hidden;pointer-events:none}"),"mobile analysis must compact the top bar, prevent viewport overflow and show only one overlay panel at a time");
...
check(styles.includes(".editorFloating{position:absolute;right:18px;top:82px;width:360px;max-width:calc(100% - 36px);max-height:75%;box-sizing:border-box")&&styles.includes("min-width:340px")&&workspace.includes("export function clampPanelTranslation(next")&&workspace.includes("function clampResultPosition(next)")&&workspace.includes("return clampPanelTranslation(next")&&workspace.includes("panel.offsetLeft")&&workspace.includes("viewport.clientWidth")&&workspace.includes("setPosition(clampResultPosition")&&workspace.includes("function moveResultPanelKey(e)")&&workspace.includes('className="editorFloatMoveHandle"')&&workspace.includes('aria-label="Mover painel de resultados"')&&workspace.includes('title="Arraste ou use setas, Home e End para mover"')&&styles.includes(".editorFloatMoveHandle{")&&workspace.includes('e.key==="Home"||e.key==="End"')&&workspace.includes("ref={resultPanel}")&&workspace.includes("data-position-x={position.x}"),"floating results container must remain pointer/keyboard movable and resizable while using deterministic viewport-clamp geometry");
