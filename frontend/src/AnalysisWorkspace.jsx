...
export default function AnalysisWorkspace({appInfo=null,executionIssue="",referenceValidating=false,selectedEngineLabels=[],file,prev,referenceFile,referencePrev,spatialRgbFile=null,spatialRgbPrev=null,onSpatialRgbFile=null,spatialRegistration=null,spatialRgbAnalysis=null,spatialPhotoViews=[],activeSpatialPhotoId=null,onAddSpatialPhotos=null,onSelectSpatialPhoto=null,onRemoveSpatialPhoto=null,spatialAutoRegistrationBusy=false,spatialAutoRegistrationStatus=null,onAutoRegisterSpatialPhotos=null,spatialRegistrationPointCount=0,photometricSummary=null,onPhotometricSummary=null,onSpatialRegistrationSample=null,spatialPathologyRecords=[],onSpatialPathologyRecords=null,onSolveSpatialRegistration=null,referenceInspectionId,referenceInspectionMeta,inspectionMeta,res,busy,progress,selected,onFile,onReferenceFile,onRun,onCancel,onSettings,error,onExport,onExportCsv,onExportMap,onExportCdm,onExportCdm3=null}){
  ...
  useEffect(()=>{
    ...
    resultOpenPreference.current=true;
    setResultOpen(true);
  ...
  const measuredDurationMs=Number(res?.metadata?.client_elapsed_ms),hasMeasuredDuration=Number.isFinite(measuredDurationMs)&&measuredDurationMs>=0;
  useEffect(()=>{
    if(busy&&!res){const now=performance.now();setStartAt(now);setElapsed(0);return}
    if(res&&hasMeasuredDuration)setElapsed(Math.floor(measuredDurationMs/1000));
    setStartAt(null);
  ...
  const pathologyVisibleCount=orderedPathologyLayers.filter(layer=>visible[pathologyEngineId+":"+layer.id]!==false).length;
  ...
  const cdmSummary=pathologyEngineId?chosen.metrics?.summary:null;
  const spatialSummary=chosen?.engine_id==="cdm_3"&&chosen?.metrics?.runtime_mode==="spatial_browser_ingestion"?chosen.metrics?.spatial_asset:null;
  ...
  function setPathologyGroupVisible(next){
    setVisible(current=>{
      ...
      for(const layer of orderedPathologyLayers)updated[pathologyEngineId+":"+layer.id]=next;
      return updated;
    });
  }
  ...
  function isolatePathologyLayer(id){
    setVisible(current=>{
      const updated={...current};
      for(const layer of orderedPathologyLayers)updated[pathologyEngineId+":"+layer.id]=layer.id===id;
      return updated;
    });
  }
  ...
  function renderOverlayStack(stackStyle=null){
    ...
    return <div className="editorOverlayStack" aria-label="Camadas de detecção sobre a imagem original" style={stackStyle||undefined}>
      ...
      {[...orderedPathologyLayers].reverse().filter(layer=>visible[pathologyEngineId+":"+layer.id]!==false).map(layer=><img className="pathologyOverlay" key={layer.id} src={"data:image/png;base64,"+layer.overlay_png_base64} alt={layer.name} data-layer-id={layer.id} style={{opacity:opacity*Number(pathologyOpacity[layer.id]??1)}}/>)}
    ...
    <div className="editorTop">
      <div className="editorBrand"><span className="editorMark">S</span><strong>SHM Studio</strong><nav className="editorMenus" aria-label="Menu do editor">
        <details className="editorMenu" name="shm-editor-menu"><summary>Arquivo</summary><div className="editorMenuPanel" role="menu">
          ...
          {kind==="2d"&&selected.length===1&&selected[0]==="cdm_1"&&<button type="button" role="menuitem" disabled={busy||referenceValidating} onClick={e=>runEditorMenuAction(e,()=>referencePicker.current?.click())}><span>Referência t0…</span></button>}
        </div></details>
        <details className="editorMenu" name="shm-editor-menu"><summary>Editar</summary><div className="editorMenuPanel" role="menu">
          <button type="button" role="menuitem" disabled={kind!=="2d"} onClick={e=>runEditorMenuAction(e,resetViewerPreferences)}><span>Restaurar visualização</span></button>
          {kind==="2d"&&referenceFile&&<button type="button" role="menuitem" disabled={busy} onClick={e=>runEditorMenuAction(e,()=>onReferenceFile(null))}><span>Remover referência t0</span></button>}
        </div></details>
      ...
      <div className="editorTopActions">
        ...
        <input ref={spatialRgbPicker} hidden type="file" accept={IMAGE_ACCEPT} onChange={e=>{const next=e.target.files?.[0]||null;e.target.value="";onSpatialRgbFile?.(next)}}/>
        <input ref={spatialMultiPicker} hidden multiple type="file" accept={IMAGE_ACCEPT} onChange={e=>{const next=[...(e.target.files||[])];e.target.value="";onAddSpatialPhotos?.(next)}}/>
        ...
        {kind==="2d"&&selected.length===1&&selected[0]==="cdm_1"&&<button disabled={busy||referenceValidating} title={referenceValidating?"Validando referência t0…":"Carregar imagem anterior para comparação temporal"} onClick={()=>referencePicker.current?.click()}><ImagePlus size={16}/> {referenceValidating?"Validando t0…":referenceFile?"t0: "+referenceFile.name:"Referência t0"}</button>}
        {kind==="3d"&&isCdm3SpatialAsset(file)&&selected.length===1&&selected[0]==="cdm_3"&&<button className={spatialPhotoViews.length?"spatialRgbAttached":""} disabled={busy} title="Adicionar até 8 fotografias para registro e texturização fotométrica multivista" onClick={()=>spatialMultiPicker.current?.click()}><ImagePlus size={16}/> {spatialPhotoViews.length?"Fotos 3D · "+spatialPhotoViews.length:"Adicionar fotos"}</button>}
    ...
    <div className="editorBody">
      <div className="editorTools" aria-label="Ferramentas">
        <div className="editorToolButtons">
          ...
          <button aria-label="Ampliar zoom" title={kind==="2d"?"Ampliar":"Zoom 2D indisponível"} disabled={kind!=="2d"} onClick={()=>changeZoom(.25)}><Plus/></button>
          <button aria-label="Reduzir zoom" title={kind==="2d"?"Reduzir":"Zoom 2D indisponível"} disabled={kind!=="2d"} onClick={()=>changeZoom(-.25)}><Minus/></button>
          <button title={kind==="2d"?"Ajustar à tela":"Ajuste 2D indisponível"} aria-label="Ajustar à tela" disabled={kind!=="2d"} onClick={fitView}><Maximize2/></button>
      ...
      {layersOpen&&<><button type="button" className="editorLayersScrim" aria-label="Fechar painel de camadas" title="Fechar camadas" onClick={()=>setLayersPanelOpen(false)}/><aside className={"editorLayers "+(selectedEngineLabels.length>=24?"engineDensityUltra":selectedEngineLabels.length>=12?"engineDensityDense":"")} data-engine-count={selectedEngineLabels.length} data-layer-width={layersWidth} style={{width:layersWidth,"--engine-count":Math.max(1,selectedEngineLabels.length)}}><div className="editorLayersContent"><div className="editorPanelTitle"><b>Camadas</b><button aria-label="Recolher painel de camadas" title="Recolher camadas" onClick={()=>setLayersPanelOpen(false)}><ChevronLeft size={17}/></button></div>
        <div className="layerRow"><span>◉</span> {kind==="2d"?"Imagem atual · t1":kind==="3d"?"Modelo 3D atual":"Arquivo atual"}</div>
        {kind==="2d"&&referenceFile&&<div className="layerRow referenceLayer"><span>○</span><span>Referência · t0 <small>{referenceFile.name}</small>{linkedReferenceCompatibility&&<em className={"referenceCompatibility "+linkedReferenceCompatibility.status} title={linkedReferenceCompatibility.refLabel}>{linkedReferenceCompatibility.text}</em>}</span><button className="layerClear" aria-label="Remover referência temporal t0" disabled={busy} title="Remover referência t0" onClick={()=>onReferenceFile(null)}><X size={13}/></button></div>}
        {kind==="3d"&&spatialPhotoViews.map(view=><div className={"layerRow referenceLayer spatialRgbLayer "+(view.id===activeSpatialPhotoId?"selected":"")} key={view.id}><span>{view.id===activeSpatialPhotoId?"◉":"○"}</span><span>{view.file?.name}<small>{view.registration?.registration?(view.registration?.method==="orb_anchor_projection_pnp"?"auto registrada":"registrada"):view.auto_registration_error?"auto falhou · manual disponível":"registro pendente"}</small><em className={"referenceCompatibility "+(view.registration?.registration?"pass":"warning")}>{view.registration?.registration?"fotometria elegível":"selecione e registre"}</em></span><button className="layerSolo" type="button" disabled={busy||view.id===activeSpatialPhotoId} onClick={()=>onSelectSpatialPhoto?.(view.id)}>Ativar</button><button className="layerClear" aria-label={"Remover "+view.file?.name} disabled={busy} title="Remover vista" onClick={()=>onRemoveSpatialPhoto?.(view.id)}><X size={13}/></button></div>)}
        ...
        {kind==="2d"&&<><p>Sobreposição global: {Math.round(opacity*100)}%</p><input aria-label="Opacidade da sobreposição" type="range" min="0" max="1" step=".05" value={opacity} onChange={e=>setOpacity(Number(e.target.value))}/></>}
        </div>
        {kind==="2d"&&<div className="editorSidebarActions"><button className="viewerReset" type="button" onClick={resetViewerPreferences}>Restaurar visualização</button></div>}
      ...
      <div className={"editorViewport "+(fileDragActive?"fileDragActive":"")} ref={surface} onDragEnter={fileDragEnter} onDragOver={fileDragOver} onDragLeave={fileDragLeave} onDrop={fileDrop}>
        ...
        {resultOpen&&(busy||results.length>0)&&<div ref={resultPanel} className="editorFloating" data-position-x={position.x} data-position-y={position.y} data-panel-width={resultPanelSize.width} data-panel-height={resultPanelSize.height||""} style={{transform:`translate(${position.x}px,${position.y}px)`,"--result-panel-width":resultPanelSize.width+"px",...(resultPanelSize.height?{"--result-panel-height":resultPanelSize.height+"px"}:{})}}>
          ...
          {busy&&<div className={"editorProgress "+(persisting?"persisting":"")}><span>{persisting?"Salvando histórico local":(progress?.current_engine||"Processando motores")+" · "+(progress?.total===100?pct+"%":(progress?.completed||0)+"/"+(progress?.total||selected.length))}</span><strong>{String(Math.floor(elapsed/60)).padStart(2,"0")}:{String(elapsed%60).padStart(2,"0")}</strong><div><i style={{width:(persisting?100:pct)+"%"}}/></div>{onCancel&&!persisting&&<button onClick={onCancel} disabled={progress?.state==="cancel_requested"}>{progress?.state==="cancel_requested"?"Cancelando…":"Cancelar"}</button>}</div>}
          {!busy&&res&&hasMeasuredDuration&&<div className="editorRunTime editorMetricRow"><span className="editorMetricLabel">Tempo da análise</span><b className="editorMetricValue">{(measuredDurationMs/1000).toFixed(2)} s</b></div>}
          {results.map(r=>{const area=Number(r.metrics?.crack_area_ratio??r.metrics?.model_crack_area_ratio);const hasArea=Number.isFinite(area)&&area>=0;const spatial=r.metrics?.runtime_mode==="spatial_browser_ingestion";const spatialPoints=Number(r.metrics?.spatial_asset?.sampled_points||0);return <button key={r.engine_id} className={"editorResultRow "+(chosen?.engine_id===r.engine_id?"active":"")} onClick={()=>{setActive(r.engine_id);setVisible(v=>({...v,[r.engine_id]:true}))}}><span className="editorResultEngine">{r.name}</span><span className="editorResultMetric"><small>{spatial?"Pontos":hasArea?"Área fissura":"Achados"}</small><b>{spatial?spatialPoints.toLocaleString("pt-BR"):hasArea?(area*100).toFixed(area<.001?3:2)+"%":(r.detections?.length||0)}</b></span><span className="editorResultMetric"><small>Latência</small><b>{Number(r.latency_ms||0).toFixed(0)} ms</b></span></button>})}
          {spatialSummary&&<div className="editorCdmSummary"><b className="editorCdmTitle">CDM-3 · ativo espacial</b><div className="editorMetricsGrid"><div className="editorMetricRow accentA"><span className="editorMetricLabel">Formato</span><b className="editorMetricValue">{String(chosen?.metrics?.source_format||"—").toUpperCase()}</b></div><div className="editorMetricRow accentB"><span className="editorMetricLabel">Pontos</span><b className="editorMetricValue">{Number(spatialSummary.sampled_points||0).toLocaleString("pt-BR")}</b></div><div className="editorMetricRow accentC"><span className="editorMetricLabel">RGB por ponto</span><b className="editorMetricValue">{spatialSummary.has_rgb?"Sim":"Não"}</b></div><div className="editorMetricRow accentD"><span className="editorMetricLabel">Canais</span><b className="editorMetricValue">{(spatialSummary.visual_channels||["elevation"]).map(x=>String(x).replace("classification","classe").replace("intensity","intensidade").replace("elevation","elevação").toUpperCase()).join(" · ")}</b></div><div className="editorMetricRow accentA"><span className="editorMetricLabel">Segmentação</span><b className="editorMetricValue">{spatialSummary.has_rgb?"visual + espacial":"geométrica / sem textura"}</b></div><div className="editorMetricRow accentB"><span className="editorMetricLabel">RGB externo</span><b className="editorMetricValue">{chosen?.metrics?.image_registration?.source?.name||"—"}</b></div><div className="editorMetricRow accentC"><span className="editorMetricLabel">Registro 2D→3D</span><b className="editorMetricValue">{chosen?.metrics?.image_registration?.state==="rgb_source_attached_pose_required"?"PnP/RANSAC pendente":chosen?.metrics?.image_registration?.state||"—"}</b></div><div className="editorMetricRow accentA"><span className="editorMetricLabel">Vistas fotométricas</span><b className="editorMetricValue">{Number(photometricSummary?.registered_views||0)} / {spatialPhotoViews.length}</b></div><div className="editorMetricRow accentB"><span className="editorMetricLabel">Cobertura textura</span><b className="editorMetricValue">{photometricSummary? (Number(photometricSummary.coverage_ratio||0)*100).toFixed(1)+"%":"—"}</b></div><div className="editorMetricRow accentC"><span className="editorMetricLabel">Patologias 3D</span><b className="editorMetricValue">{Number(spatialPathologyRecords.length||0).toLocaleString("pt-BR")}</b></div><div className="editorMetricRow accentD"><span className="editorMetricLabel">Registro métrico</span><b className="editorMetricValue">{spatialRegistration?.registration?.metric_projection_valid?"Válido":"Não validado"}</b></div><div className="editorMetricRow accentC"><span className="editorMetricLabel">Schema IFC</span><b className="editorMetricValue">{spatialSummary.schema||"—"}</b></div><div className="editorMetricRow accentD"><span className="editorMetricLabel">Entidades IFC</span><b className="editorMetricValue">{Number(spatialSummary.entity_count||0).toLocaleString("pt-BR")}</b></div></div>{spatialSummary.bounds&&<p><b>Bounds:</b> min [{spatialSummary.bounds.min.map(v=>Number(v).toFixed(3)).join(", ")}] · max [{spatialSummary.bounds.max.map(v=>Number(v).toFixed(3)).join(", ")}].</p>}{chosen?.message&&<p>{chosen.message}</p>}</div>}
          {cdmSummary&&<div className="editorCdmSummary"><b className="editorCdmTitle">{chosen?.name||"CDM"} · resumo morfológico</b><div className="editorMetricsGrid"><div className="editorMetricRow accentA"><span className="editorMetricLabel">Achados</span><b className="editorMetricValue">{cdmSummary.total_objects}</b></div><div className="editorMetricRow accentB"><span className="editorMetricLabel">Fissuras</span><b className="editorMetricValue">{cdmSummary.crack_count}</b></div><div className="editorMetricRow accentC"><span className="editorMetricLabel">Comprimento</span><b className="editorMetricValue">{Number(cdmSummary.crack_length_total_px||0).toFixed(1)} px</b></div><div className="editorMetricRow accentD"><span className="editorMetricLabel">Desplacamento</span><b className="editorMetricValue">{Number(cdmSummary.spalling_area_px2||0).toFixed(0)} px²</b></div></div>{cdmRating?.enabled&&<p>Estimativa por imagem: NT {cdmRating.NT_img} · EC {cdmRating.EC_DNIT_img} · GDE {Number(cdmRating.GDE_img||0).toFixed(2)}. Confirme em inspeção técnica.</p>}{temporal?.enabled&&<p><b>t0→t1:</b> crescimento {temporalGrowth.toFixed(temporalCalibrated?1:0)} {temporalUnit} · redução {temporalReduction.toFixed(temporalCalibrated?1:0)} {temporalUnit} · {temporalAlignment?.accepted?<>registro automático Δx={Number(temporalAlignment.dx_px||0).toFixed(0)} px, Δy={Number(temporalAlignment.dy_px||0).toFixed(0)} px · ganho {(Number(temporalAlignment.improvement||0)*100).toFixed(1)}%</>:<>alinhamento {temporal?.alignment_method==="translation_auto"?"automático sem translação aplicada":"por redimensionamento"}</>}.</p>}{temporalMetricRows.length>0&&<details className="editorTemporalMetrics"><summary>Quantificação por patologia <small>{temporalUnit}</small></summary><div className="editorTemporalTable"><span className="head">Patologia</span><span className="head">t0</span><span className="head">t1</span><span className="head">Δ</span><span className="head">Δ/t0</span>{temporalMetricRows.map(row=><React.Fragment key={row.cls}><span title={"IoU "+row.iou.toFixed(3)}>{row.label}</span><span>{row.previous.toFixed(1)}</span><span>{row.current.toFixed(1)}</span><span className={row.net>0?"positive":row.net<0?"negative":""}>{row.net>0?"+":""}{row.net.toFixed(1)}</span><span>{row.netRate==null?"—":(row.netRate>0?"+":"")+row.netRate.toFixed(1)+"%"}</span></React.Fragment>)}</div></details>}{temporalQualityLabel&&<p className={"editorTemporalQuality "+temporalQuality.status}><b>{temporalQualityLabel}</b>{temporalQuality?.metrics&&<> · sobreposição {(Number(temporalQuality.metrics.overlap_ratio||0)*100).toFixed(1)}% · Δ iluminação {(Number(temporalQuality.metrics.illumination_delta||0)*100).toFixed(1)}% · razão de nitidez {Number(temporalQuality.metrics.sharpness_ratio||0).toFixed(2)} · similaridade geométrica {Number(temporalQuality.metrics.edge_similarity||0).toFixed(2)}</>}{temporalQualityNotes.length>0&&<span> · {temporalQualityNotes.join("; ")}</span>}</p>}{temporalRegistrationWarning&&<p className="editorCdmWarning">{temporalRegistrationWarning}</p>}{chosen?.metrics?.performance_ms&&<div className="editorPerf"><b className="editorPerfTitle">Tempo real</b><div className="editorMetricsGrid performance"><div className="editorMetricRow accentDecode"><span className="editorMetricLabel">Decodificação</span><b className="editorMetricValue">{(Number(chosen.metrics.performance_ms.decode||0)/1000).toFixed(2)} s</b></div><div className="editorMetricRow accentCore"><span className="editorMetricLabel">Núcleo</span><b className="editorMetricValue">{(Number(chosen.metrics.performance_ms.core||0)/1000).toFixed(2)} s</b></div><div className="editorMetricRow accentRender"><span className="editorMetricLabel">Renderização</span><b className="editorMetricValue">{(Number(chosen.metrics.performance_ms.render||0)/1000).toFixed(2)} s</b></div><div className="editorMetricRow accentTotal"><span className="editorMetricLabel">Total</span><b className="editorMetricValue">{(Number(chosen.metrics.performance_ms.total||0)/1000).toFixed(2)} s</b></div><div className="editorMetricRow accentRuntime"><span className="editorMetricLabel">Runtime</span><b className="editorMetricValue">{chosen.metrics.runtime||"browser"}</b></div></div></div>}</div>}
          ...
          {results.length>0&&<div className="editorCompare editorViewActions">
            ...
            <details ref={exportMenu} className="editorExportMenu editorExportUnified">
              ...
              <div>
                ...
                {chosen?.engine_id==="cdm_3"&&spatialPathologyRecords.length>0&&<><span className="editorExportSection">CDM-3 · espacial</span><button onClick={()=>{showStatusNotice("Exportação CDM-3 · JSON 3D",1200);onExportCdm3?.("json")}}>JSON 3D</button><button disabled={!String(file?.name||"").toLowerCase().endsWith(".ifc")} title={!String(file?.name||"").toLowerCase().endsWith(".ifc")?"A exportação IFC anotada requer que o ativo atual seja um arquivo .ifc":"Resolver elemento hospedeiro e exportar IfcAnnotation"} onClick={()=>{showStatusNotice("Exportação CDM-3 · IFC anotado",1200);onExportCdm3?.("ifc")}}>IFC anotado</button></>}
                {cdmSummary&&<>
                  <span className="editorExportSection">{chosen?.name||"CDM"}</span>
