...
check(settingsStyles.includes(".settingsContent{width:100%;max-width:none;flex:1;min-height:0;margin:0;overflow:hidden")&&settingsStyles.includes("display:flex;flex-direction:column"),"general settings content must not scroll");
check(settingsStyles.includes(".settingsMotorsCard{display:flex;flex-direction:column;flex:1;min-height:0}"),"motor catalog card must consume the remaining viewport height");
check(settingsStyles.includes(".settingsEngines{display:grid;grid-template-columns:repeat(2,minmax(0,1fr))")&&settingsStyles.includes("flex:1;min-height:0;overflow-y:auto;overflow-x:hidden"),"engine catalog must be the only vertical scrolling region");
check(settingsStyles.includes(".settingsEngines{min-height:clamp(150px,20vh,260px)")&&settingsStyles.includes(".settingsOwnedEngine .settingsEngineDescription{display:-webkit-box;-webkit-line-clamp:2"),"mobile settings must reserve visible height for other engines while compacting the owned engine");
