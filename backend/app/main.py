...
app=FastAPI(title="SHM Vision Lab API",version="0.3.0")
...
@app.get("/")
def root():
    return {"name":"SHM Vision Lab API","status":"online","docs":"/docs","version":"0.3.0"}
...
def engines():
    ...
    for e in REGISTRY.values():
        ...
        out.append(EngineInfo(
            ...
            requires_weights=e.meta.requires_weights,recommended=e.meta.recommended,domain_mode=e.meta.domain_mode
        ))
    return out
...
async def compare(file:UploadFile=File(...),engines:str=Form("recommended")):
    ...
    async def one(i):
        async with _engine_sem:
            return await asyncio.to_thread(REGISTRY[i].run,image.copy())
    results=await asyncio.gather(*(one(i) for i in ids))
    return CompareResponse(image_width=image.width,image_height=image.height,results=results)
